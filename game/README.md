...
AI uses the same flight simulation and machine-gun ballistics as the player, with pursuit/lead, bursts, jam clearing, engine management, turning and diving attacks, overshoot attempts, terrain avoidance and bombing passes. After taking damage, an AI pilot may enter a short evasive state with frequent reversing turns and a dive when terrain clearance permits, then return to normal tactics. Bomber rear gunners have a rear-upper firing arc and hold fire for friendly aircraft in their lane. Aircraft hits use swept moving collision boxes and airframe health. Player gun cocking, heat, dispersion and 250-round belts remain intact.
