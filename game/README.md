...
AI uses the same flight simulation and machine-gun ballistics as the player, with pursuit/lead, bursts, jam clearing, engine management, turning and diving attacks, overshoot attempts, terrain avoidance and bombing passes. At the start of a sortie, scouts and fighters may choose a friendly bomber to escort. They hold a loose station above, behind and to one side of it to retain energy and spacing, but normal perception continues and a spotted enemy immediately takes priority. Pilots visibly scan when unengaged. Spotting is strongest ahead, moderate to the sides, and least reliable below or behind; a moving scan direction improves detection. A seen contact is remembered for 14 seconds, while gunfire immediately identifies the attacker so a pilot does not ignore being shot. After taking damage, an AI pilot may evade for 15–25 seconds, holding each changing turn-and-dive direction for 1–4 seconds before returning to normal tactics. Bomber rear gunners visibly follow their lead aim, scan the rear sky while idle, have a rear-upper firing arc, and hold fire for friendly aircraft in their lane. Their gun reports are audible with distance falloff. Forward guns fire at 600 RPM from 100-round belts with every third round a tracer; bomber rear guns retain 250 rounds. Gun heat now builds and dissipates at twice the original rate. The fighter has two independently operated forward guns, each with its own sight, trigger, action, heat, and ammunition.

AI re-enters after an eight-second loss delay; each replacement reuses its roster slot. An AI aircraft that makes no meaningful progress for 30 seconds while it should be moving is treated as stuck and enters the same replacement flow; intentional runway waits and servicing are exempt. Surviving AI flights also return before fuel or ammunition exhaustion, after completing a bomber sortie, after critical damage, or after a long patrol. They approach and land, roll out, taxi to parking, spend 12 seconds refuelling, rearming and repairing, then queue for another physical departure. The hangar selects aircraft and airfield without resetting the world. Ammo, bomb, airframe, and navigation displays read the same continuous world state as physics. Player aircraft on a friendly runway slowly replenish fuel and all fitted gun ammunition.

Automated checks cover continuous AI activity, replacements, bomber escort and combat priority, complete AI landing/service/relaunch circuits, directional perception and attack awareness, component hit regions and detachment, side changes, mission sequencing and persistence, roll accumulation, close-tree geometry, player bomb attribution, bomb racks/payload, swept hits, friendly-fire inhibition, and model lifecycle. These are headless checks, not a browser playtest or hardware frame-rate benchmark. AI tactics and perception remain intentionally lightweight rather than historically calibrated.
...
- Spatial left-wing, right-wing, tail and engine damage. Wing loss reduces lift and creates asymmetric roll, tail damage reduces control authority, and engine damage reduces power. Failed external parts detach, tumble and fall before any later whole-aircraft crash.
- AI pilot scanning with front/side/rear-and-below spotting weights, 14-second contact memory, immediate attacker awareness, and complete return, landing, taxi, service and relaunch behaviour.
...
Map-edge warning and a 700 m out-of-bounds margin precede an aircraft loss. Fuel-starved AI aircraft attempt a power-off landing at a friendly runway. Advanced tactics and more detailed internal systems remain future work. Bullet cover includes terrain and airfield buildings; decorative trees and village houses are not yet bullet cover. Aircraft still collide with their scenery proxies. Wreckage uses simple visual physics.
