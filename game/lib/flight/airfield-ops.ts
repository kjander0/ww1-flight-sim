...
export function stoppedRunway(s:FlightSimulation){return !s.crashed&&s.grounded&&s.velocity.length()<.7?AIRFIELDS.findIndex(f=>Math.abs(s.position.x-f.x)<31&&Math.abs(s.position.z-f.z)<520):-1;}
