...
import type { DamageComponent } from './simulation';
...
function aircraftModel(type:AircraftType,side:Team,carryBombs=true){
  ...
  const rearMount=new T.Group(),rearGunner=new T.Group(),pilot=new T.Group(),components:Record<DamageComponent,T.Group>={leftWing:new T.Group(),rightWing:new T.Group(),tail:new T.Group(),engine:new T.Group()};for(const component of Object.values(components))g.add(component);
  const box=(size:number[],pos:number[],mat:T.Material,parent:T.Object3D=g)=>{const m=new T.Mesh(new T.BoxGeometry(...size as [number,number,number]),mat);m.position.set(...pos as [number,number,number]);parent.add(m);return m;};
  box([1,.8,6*spec.length],[0,-.15,.1],body);for(const y of[-.3,1.48])for(const [component,sign] of [['leftWing',-1],['rightWing',1]] as const)box([spec.span/2,.1,1.6],[sign*spec.span/4,y,-1.4],canvas,components[component]);
  box([2.5*spec.span/8.8,.08,.9],[0,-.25,4.1*spec.length],canvas,components.tail);box([.09,1.15,.9],[0,.25,4.1*spec.length],team,components.tail);
  for(const x of[-spec.span*.35,spec.span*.35]){const wing=x<0?components.leftWing:components.rightWing;box([.08,1.8,.08],[x,.6,-1.9],dark,wing);box([.08,1.8,.08],[x,.6,-.9],dark,wing);box([1,.025,.6],[x,1.55,-1.4],team,wing);}
  const prop=components.engine;
  if(type==='bomber')for(const x of[-spec.span*.18,spec.span*.18]){
    const nacelle=box([.7,.58,1.7],[x,.38,-1.75],body,components.engine);nacelle.rotation.x=.02;
    const spinner=new T.Group();spinner.position.set(x,.38,-2.65);prop.add(spinner);box([.11,1.8,.07],[0,0,0],dark,spinner);
  }else {const spinner=new T.Group();spinner.position.set(0,0,-3.52);prop.add(spinner);box([.12,1.8,.07],[0,0,0],dark,spinner);}
  ...
  pilot.position.set(0,.34,.35);g.add(pilot);box([.34,.42,.28],[0,.05,0],new T.MeshLambertMaterial({color:'#534735',flatShading:true}),pilot);box([.25,.25,.25],[0,.34,-.03],new T.MeshLambertMaterial({color:'#9d7859',flatShading:true}),pilot);
  if(type==='bomber'){
  ...
  const bombs:T.Group[]=[];if(carryBombs)for(let i=0;i<spec.bombs;i++){const b=bombModel();b.position.copy(rackPosition(i,spec.span));g.add(b);bombs.push(b);}return{g,prop,bombs,rearMount,rearGunner,pilot,components};
}
...
export class BattleView {
  ...
  private root=new T.Group();private models=new Map<number,ReturnType<typeof aircraftModel>&{generation:number;effects:CrashEffects;camera:T.PerspectiveCamera;detached:Set<DamageComponent>}>();private parkedModels=new Map<string,{intact:T.Group;wreck:T.Group}>();
  private fallingParts:{owner:number;object:T.Object3D;velocity:T.Vector3;spin:T.Vector3;age:number}[]=[];
  ...
  constructor(private scene:T.Scene,private battle:Battle){scene.add(this.root);this.particles=new ParticleEffects(scene,battle.surface,Math.random,(x,z)=>battle.terrain.isWater(x,z));for(const p of battle.planes.slice(1)){const model=aircraftModel(p.sim.aircraftType,p.team);this.root.add(model.g);this.models.set(p.id,{...model,generation:p.generation,effects:new CrashEffects(scene,battle.surface),camera:new T.PerspectiveCamera(),detached:new Set()});}for(const target of battle.parkedPlanes){const model=aircraftModel(target.type,target.team,false),wreck=parkedWreck(target);model.g.position.copy(target.position);model.g.quaternion.copy(target.rotation);wreck.position.copy(target.position);wreck.quaternion.copy(target.rotation);wreck.visible=false;this.root.add(model.g,wreck);this.parkedModels.set(target.id,{intact:model.g,wreck});}}
  update(dt:number,alpha:number,camera?:T.Camera){
    ...
    for(const p of this.battle.planes.slice(1)){const m=this.models.get(p.id)!;
      ...
      if(m.generation!==p.generation){m.effects.reset();m.g.visible=true;m.generation=p.generation;m.detached.clear();for(const component of Object.values(m.components))component.visible=true;for(let i=this.fallingParts.length-1;i>=0;i--)if(this.fallingParts[i].owner===p.id){this.fallingParts[i].object.removeFromParent();this.fallingParts.splice(i,1);}}
      ...
      const pilotTarget=new T.Quaternion().setFromUnitVectors(new T.Vector3(0,0,-1),p.pilotLook);m.pilot.quaternion.slerp(pilotTarget,Math.min(1,dt*5));
      for(const component of p.sim.detachedComponents)if(!m.detached.has(component)){m.detached.add(component);const source=m.components[component],piece=source.clone(true);piece.position.copy(m.g.position);piece.quaternion.copy(m.g.quaternion);this.root.add(piece);source.visible=false;const side=component==='leftWing'?-1:component==='rightWing'?1:0,velocity=p.sim.velocity.clone().add(new T.Vector3(side*4,2.5,1.5).applyQuaternion(p.sim.orientation));this.fallingParts.push({owner:p.id,object:piece,velocity,spin:new T.Vector3(.7+component.length*.03,side*.8+.25,1.1-side*.35),age:0});}
    ...
    for(let i=this.fallingParts.length-1;i>=0;i--){const part=this.fallingParts[i];part.age+=dt;part.velocity.y-=9.81*dt;part.object.position.addScaledVector(part.velocity,dt);part.object.rotateX(part.spin.x*dt);part.object.rotateY(part.spin.y*dt);part.object.rotateZ(part.spin.z*dt);const floor=this.battle.surface(part.object.position.x,part.object.position.z)+.15;if(part.object.position.y<floor){part.object.position.y=floor;part.velocity.set(0,0,0);part.spin.multiplyScalar(Math.exp(-dt*5));}if(part.age>20){part.object.removeFromParent();this.fallingParts.splice(i,1);}}
